...
import { estimationReason } from '../gantt/status-utils';
...
export function RequiresEstimation({ tasks, onOpenIssue }: Props) {
  if (tasks.length === 0) {
    return null;
  }
  ...
  return (
    <section className="jeg-section">
      <div className="jeg-section__title">Требует оценки — {tasks.length}</div>
      {tasks.map((task) => (
        <div key={task.id} className="jeg-section__row">
          ...
          <div className="jeg-section__meta">
            <span className="jeg-section__reason">{estimationReason(task)}</span>
          </div>
        </div>
      ))}
    </section>
